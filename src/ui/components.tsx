...
/* ------------------------------------------------------- PassphraseInput */
/**
 * Password input with a show/hide eye toggle — blind double entry is hostile for
 * the one gate protecting all data. Accepts all input props except `type`.
 */
export function PassphraseInput({
  ...
  return (
    <div className="relative">
      <input {...props} type={show ? 'text' : 'password'} className={`${inputCls} pr-10 ${className}`} />
      <button
        type="button"
        onClick={() => setShow((s) => !s)}
        ...
        aria-pressed={show}
        tabIndex={-1}
        className="absolute inset-y-0 right-0 grid w-10 place-items-center text-ink-faint transition hover:text-ink"
      >
        {show ? (
